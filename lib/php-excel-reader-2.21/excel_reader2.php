...
//						$bgcolor = ($bgi & 0x3f80) >> 7;
				...
				case SPREADSHEET_EXCEL_READER_TYPE_BOUNDSHEET:
						...
						if ($version == SPREADSHEET_EXCEL_READER_BIFF8){
							...
							if ($chartype == 0){
								$rec_name	= $this->_encodeASCII(substr($data, $pos+12, $rec_length));
							} else {
	...
	function _encodeASCII($string){
        ...
        if ($this->_defaultEncoding){
        	switch ($this->_encoderFunction){
        		case 'iconv' : 	$result = iconv('ASCII', $this->_defaultEncoding, $string);
        						break;
        		case 'mb_convert_encoding' : 	$result = mb_convert_encoding($string, $this->_defaultEncoding, 'ASCII' );
        						break;
        	}
        }
        return $result;
